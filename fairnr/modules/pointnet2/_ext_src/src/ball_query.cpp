...
ball_nearest(at::Tensor new_xyz, at::Tensor xyz, const float radius) {
  CHECK_CONTIGUOUS(new_xyz);
  CHECK_CONTIGUOUS(xyz);
  CHECK_IS_FLOAT(new_xyz);
  CHECK_IS_FLOAT(xyz);
  ...
  if (new_xyz.type().is_cuda()) {
    query_ball_nearest_point_kernel_wrapper(xyz.size(0), xyz.size(1), new_xyz.size(1),
                                            radius, new_xyz.data<float>(), xyz.data<float>(),
                                            idx.data<int>(), dis.data<float>());
  } else {
    AT_CHECK(false, "CPU not supported");
  }
