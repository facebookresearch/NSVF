...
class PointSRNField(SRNField):
    ...
    def get_feature(self, xyz, point_feats, point_xyz):
        ...
        if self.relative_position:
            _inds = query_inds.unsqueeze(-1).expand(S, V * P, 3)
            query_xyz = point_xyz.gather(1, _inds.clamp(min=0)).view(S, V, P, -1)
            xyz = (xyz - query_xyz).masked_fill(_inds.view(S, V, P, 3) == -1, 0)
