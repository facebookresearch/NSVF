...
@register_grid("geo_drums")
def get_drums_grid(args):
    gen_args = {
...
@register_grid("geo_drums_reload")
def get_drumereload_grid(args):
    hyperparams = get_drums_grid(args)
    hyperparams += [
        hyperparam('--half-voxel-size-at', "5000,25000,75000", save_dir_key=lambda val: f'dyvox'),
        hyperparam('--reduce-step-size-at', "5000,25000,75000"),
        hyperparam('--max-update', 150000),
        hyperparam('--total-num-update', 150000, save_dir_key=lambda val: f'max100000'),
        hyperparam('--end-learning-rate', 0.0001),
    ]
    hyperparams += [
        hyperparam('--restore-file',
         "/checkpoint/jgu/space/neuralrendering/debug_new_singlev2/geo_drumsv2.fp16.single.800x800.s1.v4.geo_nerf.emb384.ss0.05.v0.4.posemb.sdfh128.raydir.r24.bg1.0.bgsg.dis.prune2500.th0.5.dyvox.maxp.60k.p16384.chk512.rgb128.0.alpha1.0.vgg1.0.l3.adam.lr_poly.max100000.lr0.001.clip0.0.wd0.0.seed20.ngpu8/checkpoint15.pt",
        )
    ]
    return hyperparams
