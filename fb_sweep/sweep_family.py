...
@register_grid("geo_family2")
def get_ignatius2_grid(args):
    gen_args = {
        'render_resolution': '540x960',
        'render_output_types': ["hit", "rgb", "depth", "normal"],
        'render_up_vector': "(0,1,0)",
        'render_path_args': "{'radius': 1.5, 'h': 0, 'axis': 'y', 't0': -2, 'r':-1}",
    }
    gen_args = json.dumps(gen_args)
    ...
    hyperparams = [
        ...
        hyperparam('--view-per-batch', 4, save_dir_key=lambda val: f'v{val}'),
        hyperparam('--valid-view-per-batch', 1),
        hyperparam('--train-views', "0..133"),
        hyperparam('--valid-views', "133..152"),

        # model arguments
        hyperparam('--arch', 'geo_nerf', save_dir_key=lambda val: val),
        hyperparam('--quantized-voxel-path', "/private/home/jgu/data/shapenet/final/tanksandtemple_family/bbvoxel0.08.txt"),
        hyperparam('--quantized-embed-dim', 384, save_dir_key=lambda val: f'emb{val}'),
...
def get_ignatius2_grid(args):
    ...
    hyperparams += [
        hyperparam('--restore-file', "/checkpoint/jgu/space/neuralrendering/debug_new_singlev3/geo_family2v1.single.1080x1920.s1.v4.geo_nerf.emb32.ss0.01.v0.08.posemb.sdfh128.raydir.r24.addpos6.bg1.0.bgsg.dis.prune2500.th0.5.dyvox.80k.maxp.p2048.chk512.rgb128.0.alpha1.0.adam.lr_poly.max100000.lr0.001.clip0.0.wd0.0.seed20.ngpu8/checkpoint15.pt")
    ]
    return hyperparams
