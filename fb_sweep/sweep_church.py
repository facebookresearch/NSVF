...
def get_ignatius_grid(args):
    ...
    hyperparams = [
        ...
        hyperparam('--view-per-batch', 8, save_dir_key=lambda val: f'v{val}'),
        hyperparam('--valid-view-per-batch', 1),
        hyperparam('--train-views', "0..125"),
        hyperparam('--valid-views', "0..125:4"),

        # model arguments
        hyperparam('--arch', 'geo_nerf', save_dir_key=lambda val: val),
        hyperparam('--quantized-voxel-path', "/private/home/jgu/data/shapenet/church/meshvoxel10.0.txt"),
        hyperparam('--quantized-embed-dim', 384, save_dir_key=lambda val: f'emb{val}'),
        hyperparam('--raymarching-stepsize', 1.25, save_dir_key=lambda val: f'ss{val}'),
        hyperparam('--voxel-size', 10.0, save_dir_key=lambda val: f'v{val}'),
