...
def get_newlego_trans_grid(args):
    ...
    hyperparams = [
        ...
        hyperparam('--train-views', "0..200"),
        hyperparam('--valid-views', "100..292:2"),
        
       # model arguments
        hyperparam('--arch', 'geo_nerf', save_dir_key=lambda val: val),
        hyperparam('--quantized-voxel-path', "/private/home/jgu/data/shapenet/lighthouse/voxel3.txt"),
        hyperparam('--quantized-embed-dim', 384, save_dir_key=lambda val: f'emb{val}'),
        hyperparam('--raymarching-stepsize', 0.125, save_dir_key=lambda val: f'ss{val}'),
        ...
        hyperparam('--transparent-background', "0.349,0.352,0.537", save_dir_key=lambda val: f'bg'),
        hyperparam('--background-stop-gradient', True, binary_flag=True, save_dir_key=lambda val: f'sg'),
...
@register_grid("geo_lighthouse_final")
def get_lighthouse_grid(args):
    gen_args = {
        'render_resolution': "400x400",
        'render_output_types': ["hit", "rgb", "depth", "normal"],
        'render_path_args': "{'radius': 10.0, 'h': 1.5, 'axis': 'z', 't0': -2, 'r':-1}",
    }
    gen_args = json.dumps(gen_args)
    ...
    hyperparams = [
        ...
        hyperparam('--valid-views', "100..300"),
        
       # model arguments
        hyperparam('--arch', 'geo_nerf', save_dir_key=lambda val: val),
        hyperparam('--quantized-voxel-path', "/private/home/jgu/data/shapenet/final/synthetic_lighthouse/bbvoxel1.2.txt"),
        hyperparam('--quantized-embed-dim', 384, save_dir_key=lambda val: f'emb{val}'),
        hyperparam('--raymarching-stepsize', 0.15, save_dir_key=lambda val: f'ss{val}'),
        hyperparam('--voxel-size', 1.2, save_dir_key=lambda val: f'v{val}'),
        hyperparam('--max-hits', 80),
        ...
        hyperparam('--pruning-every-steps', 2500, save_dir_key=lambda val: f'prune{val}'),
        hyperparam('--pruning-th', 0.5, save_dir_key=lambda val: f'th{val}'),
        hyperparam('--half-voxel-size-at', "10000,30000,75000", save_dir_key=lambda val: f'dyvox'),
        hyperparam('--reduce-step-size-at', "10000,30000,75000"),
        hyperparam('--use-max-pruning', binary_flag=True, save_dir_key=lambda val: 'maxp'),
        hyperparam('--total-num-embedding', 60000, save_dir_key=lambda val: '60k'),

        # evaluation with rendering
        hyperparam('--rendering-every-steps', 1500),
        hyperparam('--rendering-args', gen_args),
    ...
    #         "/checkpoint/jgu/space/neuralrendering/debug_new_singlev3/geo_lighthouse_finalv1.single.800x800.s1.v4.geo_nerf.emb384.ss0.2.v1.6.posemb.sdfh128.raydir.r24.bg.dis.prune2500.th0.5.dyvox.maxp.60k.p16384.chk512.rgb128.0.alpha0.0.vgg1.0.l3.adam.lr_poly.max100000.lr0.001.clip0.0.wd0.0.seed20.ngpu8/checkpoint1.pt"
    #     )
    # ]
